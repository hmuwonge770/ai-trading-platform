...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("status", sa.Enum(name="research_session_status"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("strategy_version_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("strategy_versions.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        "backtest_results",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("experiment_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("experiments.id", ondelete="CASCADE"), nullable=False, unique=True),
    ...
    op.create_table(
        ...
        sa.Column("session_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("research_sessions.id", ondelete="SET NULL")),
        sa.Column("experiment_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("experiments.id", ondelete="SET NULL")),
...
def downgrade() -> None:
    for table in ["jobs", "market_candles", "research_hypotheses", "ai_reviews", "backtest_results", "strategy_versions", "experiments", "strategies", "research_sessions"]:
        op.drop_table(table)
    bind = op.get_bind()
