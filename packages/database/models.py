...
class Experiment(Base):
    ...
    parent_experiment_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("experiments.id", ondelete="SET NULL"), index=True)
    generation: Mapped[int] = mapped_column(Integer, default=0)
    ...
    lineage_fingerprint: Mapped[str | None] = mapped_column(String(64), unique=True)
    engine_version: Mapped[str | None] = mapped_column(String(50))
    error: Mapped[str | None] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    session: Mapped[ResearchSession] = relationship(back_populates="experiments", foreign_keys=[session_id])
    parent: Mapped[Experiment | None] = relationship(remote_side=[id], foreign_keys=[parent_experiment_id])
