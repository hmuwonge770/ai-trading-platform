...
class ExperimentManager:
    """Persist immutable experiment inputs and deterministic backtest results."""

    def create(self, db: Session, config: ExperimentConfig) -> Experiment:
        experiment_fp = experiment_fingerprint(config)
        existing = db.scalar(
            select(Experiment).where(Experiment.experiment_fingerprint == experiment_fp)
        )
        if existing is not None:
            raise DuplicateExperimentError(
                f"experiment already exists: {existing.id}"
            )
