...
class PromotionRepository:
        ...
        self.db.execute(
            text(
                ...
                INSERT INTO capital_allocations
                    (strategy_version_id, environment, max_capital,
    ...
    def get_for_update(self, promotion_id: UUID) -> Promotion | None:
        ...
        if row is None:
            return None
        return self._load_promotion(row)
    ...
    def add_approval(
        ...
        *,
        ...
        self.db.execute(
            text(
                ...
                INSERT INTO promotion_approvals
                    (promotion_id, approver_id, role, decision, fingerprint,
                     capital_limit, evidence_hash)
                VALUES (:promotion_id, :approver_id, :role, :decision, :fingerprint,
                        :capital_limit, :evidence_hash)
                """
            ),
            {
        ...
        self.db.execute(
            text("UPDATE strategy_promotions SET status = :status WHERE id = :id"),
    ...
    def activate(self, promotion_id: UUID) -> Promotion:
        ...
        promotion.activate()
        self.db.execute(
            text("UPDATE strategy_promotions SET status = 'active', activated_at = :activated_at WHERE id = :id"),
    ...
    def _promotion_params(promotion: Promotion) -> dict[str, Any]:
        ...
        return {
            ...
            "strategy_version_id": str(promotion.strategy_version.strategy_version_id),
            "from_stage": promotion.from_stage.value,
            "to_stage": promotion.to_stage.value,
    ...
    def _load_promotion(self, row: Any) -> Promotion:
        ...
        strategy_version = StrategyVersion(
            ...
            fingerprint=str(row.get("strategy_fingerprint") or "0" * 64),
            configuration={},
        )
        return Promotion(
