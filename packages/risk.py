...
class RiskGateway:
    ...
    The gateway only evaluates an already-created order intent. It has no
    exchange client and cannot submit, modify, or cancel an order. Risk rules
    are deterministic so the same portfolio, order and limits always produce
    the same decision.
    """
