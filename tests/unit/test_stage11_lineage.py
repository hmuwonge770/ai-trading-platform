...
def candles(count: int = 10) -> list[MarketBar]:
    ...
    return [MarketBar(symbol="BTCUSDT", timeframe="1H", open_time=start + timedelta(hours=i), open=Decimal("100"), high=Decimal("101"), low=Decimal("99"), close=Decimal(str(100 + i)), volume=Decimal("10")) for i in range(count)]


def config(generation: int = 0, parent: str | None = None) -> ExperimentConfig:
    return ExperimentConfig(session_id=str(uuid4()), strategy_family="moving_average", strategy_config={"short_window": 2, "long_window": 4}, symbol="BTCUSDT", timeframe="1H", start_time=datetime(2025, 1, 1, tzinfo=timezone.utc), end_time=datetime(2025, 1, 2, tzinfo=timezone.utc), initial_capital=Decimal("10000"), dataset_split=DatasetSplitConfig(), generation=generation, parent_experiment_id=parent)
...
def test_root_lineage_requires_generation_zero():
    session_id = uuid4()
    validate_parent_lineage(session_id=session_id, generation=0, parent_experiment_id=None, parent_session_id=None, parent_generation=None)
    with pytest.raises(LineageValidationError, match="generation 0"):
        validate_parent_lineage(session_id=session_id, generation=1, parent_experiment_id=None, parent_session_id=None, parent_generation=None)


def test_child_lineage_requires_same_session_and_next_generation():
    session_id = uuid4()
    parent_id = uuid4()
    validate_parent_lineage(session_id=session_id, generation=1, parent_experiment_id=parent_id, parent_session_id=session_id, parent_generation=0)
...
def test_lineage_fingerprint_is_deterministic_and_parent_sensitive():
    session_id = uuid4()
    first = lineage_fingerprint(session_id=session_id, experiment_id=None, parent_experiment_id=None, generation=0, strategy_fingerprint="strategy-a", dataset_fingerprint_value="dataset-a")
    assert first == lineage_fingerprint(session_id=session_id, experiment_id=None, parent_experiment_id=None, generation=0, strategy_fingerprint="strategy-a", dataset_fingerprint_value="dataset-a")
    assert first != lineage_fingerprint(session_id=session_id, experiment_id=None, parent_experiment_id=uuid4(), generation=1, strategy_fingerprint="strategy-a", dataset_fingerprint_value="dataset-a")
